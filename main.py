...
def analyze_datasets(source_data, target_data, logger):
    """
    Analyze datasets after preprocessing stage
    ...
    # Analyze templates in source systems
    logger.info("Analyzing templates in source systems...")
    for system, support_set in source_data["source_support_sets"].items():
        template_analysis = analyze_templates(support_set, logger)
        logger.info(f"[{system}] Template Analysis:")
        logger.info(f"  - Total templates: {template_analysis.get('total_templates', 0)}")
        logger.info(f"  - Unique templates: {template_analysis.get('unique_templates', 0)}")
        logger.info(f"  - Templates per log: Min={template_analysis.get('templates_per_log_min', 0)}, Max={template_analysis.get('templates_per_log_max', 0)}, Avg={template_analysis.get('templates_per_log_avg', 0):.2f}")
        
        if template_analysis.get('templates_only_in_anomalous'):
            logger.info(f"  - Templates only in anomalous logs: {len(template_analysis.get('templates_only_in_anomalous', []))}")
            for i, template in enumerate(template_analysis.get('templates_only_in_anomalous', [])[:5]):
                logger.info(f"    {i+1}. {template}")
    
    # Analyze templates in target system
    logger.info(f"Analyzing templates in target system: {target_data.get('target_system', 'Target')}")
    target_template_analysis = analyze_templates(target_data["support_set"], logger)
    logger.info(f"[{target_data.get('target_system', 'Target')}] Template Analysis:")
    logger.info(f"  - Total templates: {target_template_analysis.get('total_templates', 0)}")
    logger.info(f"  - Unique templates: {target_template_analysis.get('unique_templates', 0)}")
    logger.info(f"  - Templates per log: Min={target_template_analysis.get('templates_per_log_min', 0)}, Max={target_template_analysis.get('templates_per_log_max', 0)}, Avg={target_template_analysis.get('templates_per_log_avg', 0):.2f}")
    
    # Analyze vocabularies
    logger.info("Analyzing vocabularies...")
    for system, vocab in source_data["source_vocabularies"].items():
        vocab_analysis = analyze_vocabulary(vocab, logger)
        logger.info(f"[{system}] Vocabulary Analysis:")
        logger.info(f"  - Vocabulary size: {vocab_analysis.get('vocab_size', 0)}")
        logger.info(f"  - Embedding dimension: {vocab_analysis.get('embedding_dim', 0)}")
    
    # Analyze target vocabulary
    target_vocab_analysis = analyze_vocabulary(target_data["target_vocab"], logger)
    logger.info(f"[{target_data.get('target_system', 'Target')}] Vocabulary Analysis:")
    logger.info(f"  - Vocabulary size: {target_vocab_analysis.get('vocab_size', 0)}")
    logger.info(f"  - Embedding dimension: {target_vocab_analysis.get('embedding_dim', 0)}")
    
    # Analyze vocabulary overlap between source and target
    logger.info("Analyzing vocabulary overlap between source and target systems...")
    for system, source_vocab in source_data["source_vocabularies"].items():
        overlap_analysis = analyze_overlap(source_vocab, target_data["target_vocab"], logger)
        logger.info(f"Overlap between {system} and {target_data.get('target_system', 'Target')}:")
        logger.info(f"  - Source vocabulary size: {overlap_analysis.get('vocab1_size', 0)}")
        logger.info(f"  - Target vocabulary size: {overlap_analysis.get('vocab2_size', 0)}")
        logger.info(f"  - Overlap size: {overlap_analysis.get('overlap_size', 0)} ({overlap_analysis.get('overlap_percentage', 0):.2f}%)")
    
    # Create analysis plots
    logger.info("Creating analysis plots...")
    plot_dir = os.path.join(analysis_dir, "plots")
    plots = create_log_analysis_plots(source_data, target_data, plot_dir)
    logger.info(f"Created {len(plots)} analysis plots in {plot_dir}")
    
    # Export dataset statistics to CSV
    logger.info("Exporting dataset statistics...")
    csv_dir = os.path.join(analysis_dir, "stats")
    csv_files = export_dataset_stats(source_data, target_data, csv_dir)
    logger.info(f"Exported {len(csv_files)} statistics files to {csv_dir}")
