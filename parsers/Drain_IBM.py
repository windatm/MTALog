...
class Drain3Parser:
    ...
    def parse_file(self, in_file, remove_cols=None, clean=False):
        ...
        with open(
            os.path.join(self.persistence_folder, "NC_templates.txt"),
            "w",
            encoding="utf-8",
        ) as writer:
    ...
    def parse_file_os(self, in_file, ab_in_file, remove_cols=None, clean=False):
        self.logger.info("Start parsing input file %s" % in_file)
        with open(in_file, "r", encoding="utf-8") as reader:
            for line in tqdm(reader.readlines()):
                line = self._pre_process(line).strip()
                # In case some of the columns are useless.
                self.parser.add_log_message(line)
        self.logger.info("Start parsing input file %s" % ab_in_file)
        with open(ab_in_file, "r", encoding="utf-8") as reader:
            for line in tqdm(reader.readlines()):
                line = self._pre_process(line).strip()
                # In case some of the columns are useless.
                self.parser.add_log_message(line)
...
if __name__ == "__main__":
    ...
    parser = Drain3Parser(
        config_file=os.path.join(PROJECT_ROOT, "conf/drain3.ini"),
