...
class OpenStackLoader(BasicDataLoader):
    def __init__(
        self,
        in_file=None,
        ab_in_file=None,
        ...
        super(OpenStackLoader, self).__init__()

        # Construct logger.
        self.logger = logging.getLogger("OpenStackLoader")
        ...
        if os.path.exists(sequence_file) and os.path.exists(label_file):
            ...
            with open(sequence_file, "w", encoding="utf-8") as writer:
                for block in self.blocks:
                    writer.write(
                        ":".join(
                            [block, " ".join([str(x)
                                             for x in self.block2seqs[block]])]
                        )
                        + "\n"
                    )
            ...
            with open(label_file, "w", encoding="utf-8") as writer:
                for block in self.block2label.keys():
                    writer.write(
                        ":".join([block, self.block2label[block]]) + "\n")
