...
from preprocessing.datacutter.SimpleCutting import cut_all, cut_by
...
if __name__ == "__main__":
    ...
    rand_state_HDFS = os.path.join(
        save_dir,
        "results/MetaLog_BH/" + dataset + "_" + parser + "/prob_label_res/random_state",
    )

    output_model_dir = os.path.join(
        save_dir, "models/MetaLog_BH/" + dataset + "_" + parser + "/model"
    )
    output_res_dir = os.path.join(
        save_dir, "results/MetaLog_BH/" + dataset + "_" + parser + "/detect_res"
    )
    ...
    train_HDFS, dev_HDFS, test_HDFS = processor_HDFS.process(
        dataset=dataset,
        parsing=parser,
        cut_func=cut_by(0.2, 0.5, 0.01),
    ...
    train_normal_HDFS = [
        x for x, inst in enumerate(train_HDFS) if inst.label == "Normal"
    ]
    normal_ids_HDFS = train_normal_HDFS[: int(0.5 * len(train_normal_HDFS))]
    label_generator_HDFS = Probabilistic_Labeling(
        min_samples=min_samples,
        min_clust_size=min_cluster_size,
        res_file=prob_label_res_file_HDFS,
        rand_state_file=rand_state_HDFS,
    )
    ...
    train_BGL, _, _ = processor_BGL.process(
        dataset=dataset,
        parsing=parser,
        cut_func=cut_all,
